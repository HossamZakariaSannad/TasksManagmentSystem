...
export default function ViewIntakes() {
    ...
    try {
      ...
      setErrorModal({
        ...
        message:
          ...
          "Failed to fetch intakes. Please try again.",
      });
      setIntakes([]);
    } finally {
      setIntakesLoading(false);
  ...
  const fetchStudents = async (intakeId) => {
    ...
    try {
      ...
      console.log(
        "Fetched students for intake_id:",
        intakeId,
    ...
    try {
      ...
      const message =
        error.response?.status === 404
          ...
          : error.response?.data?.error ||
            "Failed to delete intake. Please try again.";
          ...
          <Box>
            ...
            >
              <Typography variant="h6" fontWeight="600" color="text.secondary">
                {viewMode === "students"
                  ? `Students in ${
                      intakes.find((i) => i.id === selectedIntakeId)?.name ||
                      "Selected Intake"
                    }`
                  : `Courses in ${
                      intakes.find((i) => i.id === selectedIntakeId)?.name ||
