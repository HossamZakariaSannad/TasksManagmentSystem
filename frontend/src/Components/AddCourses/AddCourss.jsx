...
const AddCourses = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [instructorsRes, coursesRes, allCoursesRes, intakesRes] =
          await Promise.all([
  ...
  const getTrackInstructors = useCallback(
    (trackIds) => {
      if (
        !trackIds.length ||
      ...
      const commonInstructorIds = instructorIdsPerTrack.reduce(
        (common, current) => {
  ...
  const handleAssignCourseToTrack = useCallback(async () => {
    if (
      !assignCourseForm.selectedCourse ||
      !assignCourseForm.selectedTrack ||
      !assignCourseForm.selectedOption
    ) {
      setSnackbar({
        open: true,
        message: "Please select a course, track, and option",
        severity: "error",
      });
      return;
    }
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: "Course assigned to track successfully!",
        severity: "success",
      });
      handleResetForm();
  ...
  const isActionLoading =
    createCourseLoading || assignCourseToTrackLoading || loading;
    ...
    return (
      ...
      >
        <StyledCard>
          <Typography
            variant="h6"
            sx={{ mb: 2, textAlign: "center", color: "#64748b" }}
          >
  ...
  return (
    <Box sx={{ p: 3, minHeight: "100vh" }}>
      <StyledCard>
        <Typography
          variant="h4"
          sx={{ fontWeight: 700, color: "#1e1e1e", mb: 3, textAlign: "center" }}
        >
          {tabIndex === 0
            ? "Create New Course"
        ...
        {tabIndex === 0 ? (
          <Box sx={{ maxWidth: "600px", mx: "auto" }}>
            <Typography
              variant="h6"
              sx={{ fontWeight: 600, color: "#64748b", mb: 3 }}
            >
              Course Information
            </Typography>
              ...
              <Grid item xs={12}>
                <FormControl variant="outlined" fullWidth sx={{ mb: 2 }}>
                  ...
                  <Select
                    ...
                    sx={{ borderRadius: 2 }}
                    disabled={
                      instructorsLoading || newCourseForm.tracks.length === 0
                    }
                  >
                    <MenuItem value="">
                      <em>Not assigned</em>
              ...
              <Grid item xs={12}>
                <FormControl fullWidth>
                  <Autocomplete
                    ...
                    disabled={
                      fetchAvailableIntakesLoading ||
                      !newCourseForm.tracks.length
                    }
                    noOptionsText={
                      newCourseForm.tracks.length
                        ? "No intakes available for selected tracks"
                        : "Select tracks first"
                    }
                  />
                </FormControl>
              ...
              <Grid item xs={12}>
                <FormControl fullWidth>
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <StyledTextField {...params} label="Select Course" />
                    )}
              ...
              <Grid item xs={12}>
                <FormControl fullWidth>
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <StyledTextField {...params} label="Select Track" />
                    )}
                    disabled={fetchCoursesLoading}
                    noOptionsText="No tracks available"
                  />
              ...
              <Grid item xs={12}>
                <FormControl fullWidth>
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <StyledTextField {...params} label="Select Option" />
                    )}
                    disabled={fetchAllCoursesLoading}
