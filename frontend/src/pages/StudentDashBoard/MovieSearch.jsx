...
const MovieSearch = () => {
  ...
  const fetchTrending = async () => {
    if (retryCount.trending >= maxRetries) {
      setError((prev) => ({
        ...prev,
        trending: "Maximum retry attempts reached",
      }));
      return;
    }
    ...
    try {
      const res = await axios.get(
        "https://task-project-backend-1hx7.onrender.com/movie_search/trending/"
      );
      ...
      setError((prev) => ({
        ...prev,
        trending: "Failed to fetch trending movies: " + err.message,
      }));
      setTimeout(() => {
  ...
  const fetchByGenre = async () => {
    ...
    if (isNaN(genreId)) {
      ...
      if (!genreId) {
        setError((prev) => ({
          ...prev,
          genre: 'Invalid genre name or ID. Click "Show Genres" for options.',
        }));
        return;
      }
    }
    if (retryCount.genre >= maxRetries) {
      setError((prev) => ({
        ...prev,
        genre: "Maximum retry attempts reached",
      }));
      return;
    }
    ...
    try {
      const res = await axios.get(
        `https://task-project-backend-1hx7.onrender.com/movie_search/by_genre/?genre_id=${genreId}`
      );
      ...
      setError((prev) => ({
        ...prev,
        genre: "Failed to fetch genre movies: " + err.message,
      }));
      setTimeout(() => {
  ...
  const fetchTrailer = async () => {
    ...
    if (retryCount.trailer >= maxRetries) {
      setError((prev) => ({
        ...prev,
        trailer: "Maximum retry attempts reached",
      }));
      return;
    }
    ...
    try {
      const res = await axios.get(
        `https://task-project-backend-1hx7.onrender.com/movie_search/trailer/?title=${encodeURIComponent(
          trailerTitle
        )}`
      );
      ...
      setError((prev) => ({
        ...prev,
        trailer: "Failed to fetch trailer: " + err.message,
      }));
      setTimeout(() => {
  ...
  const searchMoviesByName = async (title) => {
    ...
    if (retryCount.search >= maxRetries) {
      setError((prev) => ({
        ...prev,
        search: "Maximum retry attempts reached",
      }));
      return;
    }
    ...
    try {
      const res = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=42f35605505847f2e6a5d264ac86227c&query=${encodeURIComponent(
          title
        )}`
      );
      ...
      setError((prev) => ({
        ...prev,
        search: "Failed to search movies: " + err.message,
      }));
      setTimeout(() => {
  ...
  return (
    <Box className="movie-search-container">
      <Card elevation={3} sx={{ borderTop: "4px solid #1976d2", mb: 4 }}>
        <CardHeader
          title="Movie Search Explorer"
          avatar={<FiFilm style={{ color: "#1976d2" }} />}
          ...
          <Box>
            ...
            <Box display="flex" gap={2} mb={2} flexWrap="wrap">
              <TextField
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && searchMoviesByName(e.target.value)
                }
                sx={{ flex: 1, minWidth: 200 }}
              ...
              <Button
                ...
                onClick={() => {
                  const input = document.querySelector(
                    'input[aria-label="Search movies by title"]'
                  ).value;
