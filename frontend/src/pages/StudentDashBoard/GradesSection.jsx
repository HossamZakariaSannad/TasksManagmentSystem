...
const GradesSection = ({ grades = [], assignments = [], courses = [] }) => {
  ...
  const [gradeSubmissionDateFilter, setGradeSubmissionDateFilter] =
    useState("");
  ...
  const filterGrades = () => {
    ...
    return grades.filter((grade) => {
      ...
      const matchingAssignment =
        assignments.find((assignment) => assignment?.id === grade.assignment) ||
        {};
      const courseName = matchingAssignment.course_name || "";
      const matchesCourse =
        gradeCourseFilter === "all" || courseName === gradeCourseFilter;
      const matchesSubmissionDate =
        !gradeSubmissionDateFilter ||
        (grade.submitted_at &&
          new Date(grade.submitted_at).toISOString().slice(0, 10) ===
            gradeSubmissionDateFilter);
    ...
    <ErrorBoundary>
      <Card elevation={3} sx={{ borderTop: "4px solid #1976d2" }}>
        <CardHeader
          ...
          avatar={<FiAward style={{ color: "#1976d2" }} />}
              ...
              {filterGrades().length > 0 ? (
                <Grid container spacing={2}>
                  {filterGrades().map((grade, index) => {
                    ...
                    const courseName =
                      matchingAssignment.course_name || "No Course Name";
                    const assignmentTitle =
                      matchingAssignment.title || "No Assignment Name";
                    return (
