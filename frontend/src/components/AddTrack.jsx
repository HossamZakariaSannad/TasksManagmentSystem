...
const TrackManagement = () => {
  ...
  const supervisors = allStaff.filter(
    (s) => s.role === "supervisor" && s.branch?.id === managerBranch.id
  );
  ...
  const handleSubmit = async (e) => {
    ...
    const duplicate = filteredTracks.find(
      (t) => t.name === trimmedName && t.track_type === formData.track_type
    );
    if (duplicate && (!editingId || duplicate.id !== editingId)) {
      setLocalError(
        "A track with this name and type already exists in your branch."
      );
      return;
    }
    if (!trimmedName) {
            ...
            <TableBody>
              {filteredTracks.map((t) => (
                <TableRow key={t.id}>
                  ...
                  <TableCell>
                    {t.supervisor
                      ? supervisors.find((s) => s.id === t.supervisor)?.username
                      : "None"}
                  </TableCell>
                  <TableCell>
